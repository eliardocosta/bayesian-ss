ssPoiDP1 <- function(lam0, phi, w, rho, alpha, len.max, inc = 1E2 , R = 1E2) {
  cat("Call for PoiDP \n")
  ...
  while (len.med > len.max) {
    ...
    for (i in 1:R) {
      ...
      x <- rpois(n, w*lam)
      obj.ppost <- ppostPoiDP(x = x, w = w, phi = phi, lam0 = lam0, alpha = alpha, nburn = 1E3, nsam = 5E2)
  ...
  # SEGUNDO LOOP COM INCREMENTO IGUAL A 1
  n <- n-inc
  len.med <- len.max+1
  while (len.med > len.max) {
    n <- n+1
    ...
    for (i in 1:R) {
      ...
      lam <- sample(obj.dpriori$lam, n, replace = TRUE, prob = obj.dpriori$ddist)
      x <- rpois(n, w*lam)
      obj.ppost <- ppostPoiDP(x = x, w = w, phi = phi, lam0 = lam0, alpha = alpha, nburn = 1E3, nsam = 1E3)
      obj.dpost <- extract.prob(obj.ppost)
      vals <- obj.dpost$lam
      probs <- obj.dpost$ddist
      conj.vals <- numeric()
